...
fetch('http://localhost:3000/api/v3/moving-stations/measurements')
  .then(response => response.json())
  .then(data => {
    // Extract the pitch and roll values from the JSON response
    const pitch = data.pitch;
    const roll = data.roll;

    // Store the values in the sideTilt and frontTilt variables
    sideTilt = roll;
    frontTilt = pitch;

    // Display the values (optional)
    console.log('sideTilt:', sideTilt);
    console.log('frontTilt:', frontTilt);
  })
  .catch(error => {
